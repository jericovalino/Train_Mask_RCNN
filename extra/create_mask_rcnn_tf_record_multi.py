...
mask_pixel = {'pencil':194, 'stapler':26, 'scissors':76}
...
def dict_to_tf_example(data, filename,
  ...
  if 'object' in data:
    for obj in data['object']:
      ...
      mask_remapped = (mask_np_black == mask_pixel[class_name]).astype(np.uint8)
...
def create_tf_record(output_filename,
  ...
  with contextlib2.ExitStack() as tf_record_close_stack:
    ...
    for idx, example in enumerate(examples):
      ...
      xml_dir_path = os.path.join(annotations_dir, 'xmls')
      xml_path = os.path.join(xml_dir_path, example + '.xml')
